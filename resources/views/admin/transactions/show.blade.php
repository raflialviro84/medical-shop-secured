...
@section('header')
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.transactions.index') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
...
<div class="bg-white">
    <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-2 border-t border-gray-200">
                <dl class="divide-y divide-gray-200">
                    <div class="py-3 flex justify-between text-sm">
                        ...
                        <dd class="text-gray-900 ml-6">{{ $transaction->user->full_name ?? $transaction->user->username }}</dd>
                    </div>
                    <div class="py-3 flex justify-between text-sm">
                        <dt class="text-gray-500">Email</dt>
                        <dd class="text-gray-900 ml-6">{{ $transaction->user->email }}</dd>
                    </div>
                    @if($transaction->user->contact)
                        <div class="py-3 flex justify-between text-sm">
                            <dt class="text-gray-500">Contact</dt>
                            <dd class="text-gray-900 ml-6">{{ $transaction->user->contact }}</dd>
                        </div>
                    @endif
                    @if($transaction->user->address)
                        <div class="py-3 flex justify-between text-sm">
                            <dt class="text-gray-500">Address</dt>
                            <dd class="text-gray-900 ml-6">{{ $transaction->user->address }}</dd>
                        </div>
                    @endif
                    @if($transaction->user->city)
                        <div class="py-3 flex justify-between text-sm">
                            <dt class="text-gray-500">City</dt>
                            <dd class="text-gray-900 ml-6">{{ $transaction->user->city }}</dd>
                        </div>
                    @endif
                </dl>
            </div>
                            ...
                            <div class="ml-4 flex-1 flex flex-col">
                                <div>
                                    <div class="flex justify-between text-base font-medium text-gray-900">
                                        <h3>
                                            <a href="{{ route('admin.products.edit', $detail->product) }}">{{ $detail->product->product_name }}</a>
                                        </h3>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                This transaction is pending payment from the customer.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                Payment received. This order is ready to be shipped.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-purple-700">
                                This order has been shipped. Waiting for the customer to mark it as received.
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <!-- Download Invoice -->
        <div class="mt-6">
            <a href="{{ route('admin.transactions.invoice', $transaction) }}" target="_blank" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
